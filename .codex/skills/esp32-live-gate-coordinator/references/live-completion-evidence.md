...
- transcript proves `hello`, `state_get`, `peer_list`, `diag_get`, `fw_inventory`, `msg_post`, `msg_pull`, `msg_search`, `msg_ack`, `download_list`, `download_status`, `otap_status`, and non-executing `otap_intent`
- transcript reports three `espnow-enc` peers, zero serial errors, and moving RX/TX/ACK counters
- vision-gate output passes required visible views including Downloads, OTAP, and disabled unsafe controls
