...
| XBee lane | xctu-installed-host-only-live-blocked | source-backed study plus offline tooling and host GUI install proof | `SRC-DIGI-XBP9B-DPUT-001`, `SRC-DIGI-XBEE-PRO-900HP`, `SRC-DIGI-XBEE-900HP-USER-GUIDE`, `SRC-WAVESHARE-XBEE-USB-ADAPTER`, `SRC-LOCAL-XBEE-READONLY-PROBE-2026-05-18`, `SRC-LOCAL-XBEE-RADIO-STUDY-2026-05-29`, `SRC-LOCAL-XCTU-INSTALL-PROOF-2026-05-29` | Candidate XBee and adapter sources exist; the study CLI can inventory host/tool state without opening serial ports, compare readback/target JSON offline, emit blocked write plans, and now detect the installed per-user XCTU `XCTU-NG` path. XCTU 6.5.13.2 host install proof and first-run change-log evidence are recorded. | Adapter serial path, UART voltage, DIN/DOUT routing, settings backup, legal/antenna constraints, future update prompts, and read-only Tier A/Tier B evidence. | Use XCTU as a reference GUI only; run read-only XBee bench proof only after current adapter identity is verified and a live gate is opened. |
