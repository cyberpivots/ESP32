...
- Subagent lifecycle cleanup is recorded under
  `SRC-LOCAL-SUBAGENT-LIFECYCLE-CLEANUP-2026-06-01`. The repo now requires
  Tier 2/Tier 3 coordinators to inspect completed agents before spawning,
  collect reviewer output with `wait_agent`, close completed/stale agents with
  `close_agent`, close agents before fallback/final, and record fallback only
  after cleanup attempt or unavailable/unsafe lifecycle state. Repo hooks and
  audits remain advisory; runtime slot release still depends on actual parent
  `close_agent` calls and remains advisory only under `bypassPermissions`.
...
- Four-relay KY-040 BBS LCD menu PF0530H live gate is recorded under
  ...
  `BBS_MENU_HB`.
- Four-relay KY-040 BBS LCD menu PF0530L live gate is recorded under
  `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530L-LIVE-2026-05-31`. It
  passed COM6 identity, rollback backup, write-flash, separate verify-flash,
  and read-only monitor with LCD init, BBS ready, cursor/render, page/glyph
  auto-demo coverage, and heartbeat proof. Physical interaction remains
...
- The Tier 3 COM6 attended interaction proof is recorded under
  ...
  monitor missed user actuation, but the restarted 120 second read-only monitor
  captured 768 `ENC_RAW`, 456 `ENC_EV`, six `BBS_MENU_STEP`, 11
  `BBS_MENU_SELECT`, both menu-step directions, short and long button
  selections, 59 heartbeats, 135 renders, and no crash or unsafe-open markers.
  PF0530L serial/menu physical interaction is accepted on retry.
- The LCD glyph and electrical acceptance attempt is recorded under
  ...
  identity passed, and the first 150 second read-only monitor again captured
  PF0530L readiness plus strong encoder/menu interaction evidence with zero
  crash or unsafe-open markers. The follow-up read-only reset/monitor hit an
  LCD readiness stop gate: `LCD_INIT_FAIL stage=probe detail=scan-error`
  persisted for the full 150 second monitor. User-reported follow-up evidence shows
  a bi-directional LCD level converter with LCD-side 4.73 V on `VCC`/`SDA`/
  `SCL`, visible LCD life, and KY-040 `+`/idle `CLK`/idle `DT`/idle `SW` at
  3.3 V with affirmative drop-low/press-low responses, then ESP32-side/LV-side
  `SDA`/`SCL` at 3.3 V. A post-LV-side read-only retry restored
  `LCD_INIT_OK`, BBS LCD/input readiness, all 13 page names, all five glyph
  banks, and zero unsafe markers. User visual confirmation accepts four
  readable rows, visible page changes, and readable custom glyph/widget pages.
  User final DMM confirmation accepts the remaining continuity, KY-040 toggle,
  and current-margin checks for this gate. Direction-label expectations,
  relay/load/mains work, XBee/RF expansion, future flash, serial writes,
  wiring-under-power, deployment readiness, publication, and broader hardware
  acceptance remain closed outside this gate.
- Four-relay KY-040 BBS LCD menu PF0530M is recorded under
  `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530M-2026-06-01` as the
  historical non-live source/test continuation after the PF0530L LCD visual/electrical
  gate. PF0530M keeps the bridge closed, keeps GPIO13/GPIO14/GPIO32
  input-only, keeps LCD writes display-only, adds operational status pages,
  bridge-closed display, diagnostic/error rows, editable widget rows, glyph
  bank mapping, and host-side page wrapping/short-press/long-press/display
  bound tests. PF0530M has not been flashed.
- Four-relay KY-040 BBS LCD menu PF0530N scrolling/XML is recorded under
  `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530N-SCROLLING-XML-2026-06-01`
  as the source/test update after PF0530M. PF0530N keeps the
  bridge closed, keeps GPIO13/GPIO14/GPIO32 input-only, keeps LCD writes
  display-only on GPIO21/GPIO22, and adds build-time `bbs_lcd_menu.v1` XML,
  generated static firmware/simulator menu definitions, `bbs_lcd_render.v2`
  viewport metadata, scroll-list item navigation, grouped multi-row items,
  selected-row marquee timing, and a separate table glyph bank.
- Four-relay KY-040 BBS LCD menu PF0530N live is recorded under
  `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530N-LIVE-2026-06-01`.
  The COM6 gate recorded safe-state authority, 14/14 conditional reviewer
  approval, refreshed validation, same-session COM6 ESP32 identity, full 4 MB
  rollback backup/hash, recovery command, pinned artifacts, write-flash,
  separate verify-flash, read-only monitor with `writes_sent=false`, PF0530N
  readiness/schema/render/heartbeat proof, zero crash/unsafe markers, and
  cleanup proof. Physical LCD readability of PF0530N scroll-list/table pages
  and physical encoder/button input remain unproven.
...
- The row-0 `PF0530B` diagnostic has been written/verified to COM6, and the
  ...
  is accepted. PF0530E r5 is the GPIO-level input proof, and PF0530F was the
  LCD menu-proof source branch; its COM6 flash/verify gate passed, but
...
| LAN DHCP/current peer remap recovery | implemented-validated | read-only preflight and cleanup proof | `SRC-LOCAL-ESPNOW-LAN-DHCP-CURRENT-REMAP-2026-05-25` | Post-cleanup preflight returned `ok:true` for Pi `192.168.200.153`, coordinator `/dev/ttyUSB0` MAC `<redacted-mac>`, and peers `peer01=COM6`, `peer02=COM10`, `peer03=COM12`. | No bridge, Win31/OPCON, BBS, prepare, flash, erase, monitor, or radio proof was run in this pass. | If a new proof run is requested, start from a fresh read-only preflight on the current LAN mapping. |
...
| Full-service mesh discovery Gate M1 | accepted-host-simulator-only | ADR/source ledger plus host tests | `SRC-LOCAL-ESPNOW-FULL-SERVICE-MESH-DISCOVERY-2026-05-27`, `SRC-ESP-IDF-WIFI-MESH`, `SRC-ESP-IDF-RF-COEXIST`, `SRC-ANDROID-BLE-OVERVIEW`, `SRC-ANDROID-BLE-GATT-CONNECT`, `SRC-ANDROID-BLUETOOTH-PERMISSIONS` | Accepted `ADR-0009` defines `mesh_discovery.v1`, host topology/service/capability/BLE-Android metadata, healing-event shape, compact bridge summaries, recursive secret-field rejection, 512-byte bridge bounds, runtime summary inclusion, and unchanged Gate F radio service codes. | No live mesh proof, BLE proof, Android app proof, router/admin policy, or firmware implementation. | Superseded for DOS-C companion status by Gate M2-A and for firmware mapping design by Gate M3. |
| Full-service mesh discovery Gate M2-A DOS-C companion | implemented-host-only | paired DOS-C commit, task/handoff, source ledger, and focused host tests | `SRC-LOCAL-ESPNOW-FULL-SERVICE-MESH-DISCOVERY-GATE-M2A-DOSC-2026-05-27`, `SRC-LOCAL-ESPNOW-FULL-SERVICE-MESH-DISCOVERY-2026-05-27` | DOS-C commit `62c4db6` adds read-only `discovery_snapshot`, `discovery_events`, `service_catalog`, and `capability_report` bridge/operator support, Win31 Network/Services summaries, ASCII schema-versioned response parsing, capped event/service rows, 512-byte bridge bounds, and no coordinator serial ABI expansion. Focused Gate M2-A bridge/operator tests passed; full DOS-C Win31/scaffold suites were blocked by unrelated dirty Star Trek fullscreen-fill worktree changes. | No live mesh proof, BLE proof, Android app proof, router/admin policy, PCAP, serial-write expansion, or hardware proof. | Superseded for firmware mapping design by Gate M3; live proof requires a separate future Tier 3 gate with same-session evidence and recovery path. |
| Full-service mesh discovery Gate M3 design review | accepted-design-only | architecture note, task/handoff, source ledger, and reviewer quorum | `SRC-LOCAL-ESPNOW-FULL-SERVICE-MESH-DISCOVERY-GATE-M3-DESIGN-REVIEW-2026-06-05`, `SRC-LOCAL-ESPNOW-FULL-SERVICE-MESH-DISCOVERY-2026-05-27`, `SRC-LOCAL-ESPNOW-FULL-SERVICE-MESH-DISCOVERY-GATE-M2A-DOSC-2026-05-27`, `SRC-ESP-IDF-WIFI-MESH`, `SRC-ESP-IDF-RF-COEXIST` | M3 records a source-backed mapping matrix from ESP-WIFI-MESH concepts into existing `mesh_discovery.v1` targets and future M4 fixture requirements for event vectors, 512-byte bridge bounds, secret rejection, live-action rejection, queue/custody isolation, and schema/ABI invariance. | No firmware runtime implementation, firmware persistence, build proof, live mesh proof, BLE proof, Android app proof, router/admin policy, flash, serial-write expansion, PCAP, release, or publication. | Next safe step is separately gated M4 build-only firmware adapter proof; M5 live mesh remains Tier 3 only. |
| BBS UI system operation improvement program | ui0-m2b-implemented-host-only | Tier 2 plan/source ledger/task/handoff, read-only quorum, and paired DOS-C host proof | `SRC-LOCAL-BBS-UI-SYSTEM-OPERATION-PROGRAM-2026-05-28`, `SRC-LOCAL-BBS-UI-UI0-M2B-HOST-SLICE-2026-05-28`, `SRC-LOCAL-WIN31-DASHBOARD-INTERFACE-IMPROVEMENT-2026-05-27`, `SRC-LOCAL-CLIENT-UI-LIVE-GATE-2026-05-24`, `SRC-LOCAL-ESPNOW-FULL-SERVICE-MESH-DISCOVERY-GATE-M2A-DOSC-2026-05-27`, `SRC-LOCAL-ESPNOW-FULL-SERVICE-MESH-DISCOVERY-GATE-M3-DESIGN-REVIEW-2026-06-05` | UI-0 now has a ranked source-backed operator packet. M2-B now has host-only Network/Services UX proof tied to DOS-C commit `7f0b5df`; DOS-C `m2a.discovery.v1` companion output is distinguished from ESP32 host `mesh_discovery.v1`, and source tests preserve 512-byte/read-only/no-secret/no-serial-ABI boundaries. M3 now records design-only firmware mapping requirements. | No Client-1 simulated/static proof, Client-2 live read-only Wi-Fi proof, dummy-output proof, copied screenshot proof, live browser proof, runtime public API, firmware ABI/runtime/build proof, bridge ABI, serial ABI, Gate F service-code map, `mesh_discovery.v1` schema change, or Win31 transport change is accepted. | Continue with Client-1 static/simulated browser proof or separately gated M4 build-only adapter proof; any live proof requires a separate future Tier 3 gate. |
...
| Subagent lifecycle cleanup | implemented-advisory-process | governance docs, hook text, `SubagentStop` reminder, scaffold audit, tests, source ledger, task, and handoff | `SRC-LOCAL-SUBAGENT-LIFECYCLE-CLEANUP-2026-06-01` | Tier 2/Tier 3 reviewer quorum now includes inspect-before-spawn, `wait_agent`, `close_agent`, close-before-fallback/final, and fallback-only-after-cleanup-attempt rules. | Repo hooks/audits cannot guarantee Codex runtime slot release, and `bypassPermissions` remains advisory only. | Future quorum work must actually close completed/stale reviewers before spawning replacements, falling back, or finalizing. |
| Always-on subagent process enforcement | implemented-advisory-process | governance docs, hook text, prompt registry, scaffold audit, tests, source ledger, task, and handoff | `SRC-LOCAL-ALWAYS-ON-SUBAGENT-PROCESS-ENFORCEMENT-2026-06-02` | Safe non-trivial Tier 1 mutation and Tier 2/Tier 3 reviewer quorum now require a mandatory attempt to use project-local read-only subagents when available and safe. Local role-lens fallback must record whether subagents were unavailable, unsafe, or blocked by higher-priority tool policy and must preserve lifecycle cleanup. | Repo hooks/audits cannot guarantee future runtime subagent availability or slot cleanup, and `bypassPermissions` remains advisory only. | Future non-trivial mutation should attempt subagents first, close completed/stale reviewers before fallback/final, and avoid `/etc/codex` or admin-strict changes unless explicitly requested. |
...
| XBee lane | tier-a-no-serial-status-accepted-physical-identity-open | source-backed study, host XCTU install proof, selected-port programming evidence, bidirectional benign RF proof, corrected ESP32 bridge failure evidence, permanent bridge firmware, COM6-only flash evidence, bridge RF proof, and Task 0182 no-serial status packet | `SRC-DIGI-XBP9B-DPUT-001`, `SRC-DIGI-XBEE-PRO-900HP`, `SRC-DIGI-XBEE-900HP-USER-GUIDE`, `SRC-WAVESHARE-XBEE-USB-ADAPTER`, `SRC-DIGI-XCTU-LOCAL-DISCOVERY-2026-05-29`, `SRC-DIGI-XBEE-900HP-BD-2026-05-29`, `SRC-LOCAL-XBEE-READONLY-PROBE-2026-05-18`, `SRC-LOCAL-XBEE-RADIO-STUDY-2026-05-29`, `SRC-LOCAL-XCTU-INSTALL-PROOF-2026-05-29`, `SRC-LOCAL-XBEE-READONLY-LIVE-GATE-2026-05-29`, `SRC-LOCAL-XBEE-TWO-DEVICE-READONLY-STUDY-2026-05-29`, `SRC-LOCAL-XBEE-SELECTED-PORT-PROGRAMMING-2026-05-29`, `SRC-LOCAL-XBEE-OTA-LINK-PROOF-2026-05-29`, `SRC-LOCAL-XBEE-ESP32-UART-WIRING-PLAN-2026-05-29`, `SRC-LOCAL-ESP32-CONNECTED-XBEE-COM15-LIVE-TEST-2026-05-29`, `SRC-LOCAL-CORRECTED-ESP32-COM6-PEER-COM15-LIVE-TEST-2026-05-30`, `SRC-LOCAL-ESP32-XBEE-UART-BRIDGE-FLASH-RETEST-2026-05-30`, `SRC-LOCAL-XBEE-TIER-A-NO-SERIAL-IDENTITY-EVIDENCE-2026-06-05` | User-selected `COM15` and `COM6` were programmed to `AP=2`, `AO=0`, `EE=1`, and earlier exchanged benign `link_probe` API payloads in both directions. Corrected evidence later proved `COM6` was the ESP32 and `COM15` was the peer XBee for that gate; `COM6` did not expose XBee API frames before bridge firmware. The permanent bridge maps UART0 host `115200` to UART2 XBee `9600` on GPIO17/GPIO16 with no app logging in the copy loop; it was flashed to `COM6` only, then passed redacted COM6 local-AT readback, COM15 peer readback, and corrected bidirectional benign `link_probe` RF proof. Task 0182 accepts only no-serial status and does not complete Tier A or prove current physical adapter identity. | Current two-adapter physical identity, second one-at-a-time adapter delta, adapter markings, antenna state, physical isolation notes, voltage/header/carrier facts, Tier B read safety, XCTU/XBee Studio readiness, deployment range, throughput, relay command acceptance, source address allowlisting integration, antenna/regulatory deployment review, measured rail-current margin for broader hardware expansion, load/mains readiness, and future XBee setting-write authority remain open. | Continue with the second one-at-a-time adapter delta plus physical markings, antenna, isolation, voltage/header/carrier, recovery, and cleanup evidence before Tier A completion or Tier B/XCTU consideration. Keep bridge firmware history scoped to its named proof; reopen range/throughput, relay command payloads, future radio setting writes, or load/mains only with separate gates. |
...
| Four-relay rotary encoder LCD menu input | PF0530R-live-readiness-input-open | input-only firmware/docs/audit lineage through PF0530E GPIO-level input proof, PF0530L LCD/menu UX source plus COM6 live flash/verify/monitor evidence, accepted PF0530L retry interaction, accepted PF0530L LCD glyph/electrical gate, PF0530N scrolling/XML source/live gates, PF0530O real-menu calibration source/live gates, PF0530P debounce calibration source/live gates, PF0530Q quiet-window source/live gates, PF0530R detent-gated source/build proof, and PF0530R COM6 live write/verify/readiness proof | `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530R-LIVE-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530R-DETENT-CAL-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530Q-LIVE-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530Q-QUIET-CAL-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530P-LIVE-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530P-DEBOUNCE-CAL-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530O-LIVE-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530O-REAL-MENU-CAL-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530N-LIVE-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530N-SCROLLING-XML-2026-06-01`, `SRC-LOCAL-LCD-GLYPH-ELECTRICAL-ACCEPTANCE-2026-05-31`, `SRC-LOCAL-TIER3-COM6-ATTENDED-INTERACTION-PROOF-2026-05-31`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530L-LIVE-2026-05-31`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530L-2026-05-31`, `SRC-LOCAL-FOUR-RELAY-KY040-SERIAL-PINTRACE-PF0530E-2026-05-30`, `SRC-LOCAL-ESP32-XBEE-UART-BRIDGE-FLASH-RETEST-2026-05-30`, `SRC-ESP-IDF-GPIO`, `SRC-ESP-IDF-PCNT`, `SRC-ESPRESSIF-KNOB-1-0-2`, `SRC-ESPRESSIF-BUTTON-4-1-6`, `SRC-BOURNS-PEC11R` | PF0530L serial/menu interaction, `ENC_RAW`, `ENC_EV`, `BBS_MENU_STEP`, `BBS_MENU_SELECT`, LCD visual/glyph readability, low-side I2C/KY-040 voltage domain, and user-confirmed continuity/toggle/current-margin evidence are accepted for the closed LCD/DMM gate. PF0530N/PF0530O/PF0530P/PF0530Q records cover XML-generated menu behavior, real-menu calibration, debounce calibration, quiet-window calibration, COM6 readiness/render proof, and PF0530Q write/verify/read-only proof. The user later reported PF0530Q works but is not stable. PF0530R source/build proof adds `cal=detent-v4`, 8 ms A/B candidate hold, 15 ms quiet window, `FR_ENCODER_TRANSITIONS_PER_STEP 2`, 90 ms step lockout, detent A/B `3` gating, and detent return/step/partial telemetry. PF0530R live proof adds COM6 identity, full rollback backup, write-flash, separate verify-flash, reset/read-only monitor readiness proof, zero bad render rows, zero crash/unsafe markers, and cleanup proof. | Stable physical encoder/button acceptance remains open because the PF0530R 150 second attended read-only monitor captured zero `ENC_RAW`, zero `ENC_EV`, zero `BBS_MENU_STEP`, zero `BBS_MENU_SELECT`, and zero `ENC_FILTER`. Final BBS/XBee payload mapping, relay/load/mains readiness, and broader deployment acceptance remain outside this gate. | Next useful action is operator visual/input report on the currently flashed PF0530R image, or a read-only interaction monitor with confirmed actuation. Any future COM6 flash, serial write, RF/XBee write, relay action, wiring change, DMM/current check, or load/mains work needs a fresh Tier 3 gate. |
| Four-relay rotary encoder LCD menu input | PF0530S-raw-liveness-accepted-stability-open | active raw-liveness source/build continuation after PF0530R zero-input readiness proof plus COM6 PF0530S live write/verify/reset/attended proof | `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530S-LIVE-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530S-RAW-LIVENESS-CAL-2026-06-01`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530R-LIVE-2026-06-01`, `SRC-ESP-IDF-GPIO`, `SRC-ESP-IDF-PCNT`, `SRC-ESPRESSIF-KNOB-1-0-2`, `SRC-ESPRESSIF-BUTTON-4-1-6` | PF0530S keeps GPIO13/GPIO14/GPIO32 input-only, keeps LCD GPIO21/GPIO22 display-only, adds `ENC_BASE`, `ENC_GPIO_CONFIG`, GPIO config dump, one-second `ENC_LEVEL_HB`, raw/ISR/queue/poll counters, and transition-responsive stepping with `cal=raw-live-v5`. COM6 write-flash and separate verify-flash passed; reset boot proof captured PF0530S readiness, baseline/config/heartbeat telemetry, and zero crash/unsafe markers. The attended read-only monitor captured 450 A/B raw events, 84 switch raw events, GPIO13/GPIO14/GPIO32 ISR events, 27 clockwise steps, 21 counterclockwise steps, 14 short selects, 4 long selects, visible LCD/menu response, no serial byte writes, zero crash/unsafe markers, and cleanup proof. | Full rotary stability remains open because the attended scan also recorded 15 invalid transitions, 31 A/B suppressions, five step-lockout filters, and final heartbeat `queue_drop=57`. Final BBS/XBee payload mapping, relay/load/mains readiness, and broader deployment acceptance remain outside this gate. | Next useful action is bounded decoder/queue/debounce stability tuning or a longer acceptance monitor under a separate gate; raw input visibility is no longer the blocker. |
| Four-relay rotary encoder LCD menu input | PF0530U-written-verify-flashed-actuation-proof-pending | responsive-v7 source/live continuation after PF0530T restored raw/select proof but emitted too few rotation steps | `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530U-LIVE-2026-06-02`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530U-RESPONSIVE-V7-2026-06-02`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530T-RESPONSIVE-2026-06-02`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530S-LIVE-2026-06-01`, `SRC-ESP-IDF-GPIO` | PF0530U keeps GPIO13/GPIO14/GPIO32 input-only and LCD GPIO21/GPIO22 display-only, changes the active image to `PF0530U`, makes polling authoritative at 2 ms, disables interrupt telemetry and raw edge logging, emits detent-gated steps after one transition with 25 ms lockout, and reports `cal=responsive-v7`. Focused tests, generated menu check, firmware scaffold audit, scaffold verification, ESP-IDF v6.0.1 no-flash build, COM6 identity, 4 MB rollback backup, staged hashes, write-flash, separate parameter-matched verify-flash, and reset/readiness proof passed. | Two PF0530U post-flash read-only monitor windows captured zero `ENC_EV`, zero `BBS_MENU_STEP`, and zero `BBS_MENU_SELECT`, so final PF0530U physical actuation proof remains open. | Next useful action is a read-only monitor while the operator actively turns and presses the encoder; do not reflash unless a new gate explicitly authorizes it. |
| Four-relay rotary encoder LCD menu input | PF0530V-user-confirmed-functional-telemetry-characterization-open | PCNT source/live continuation after PF0530U did not deliver physical input proof, followed by user/operator acceptance | `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530V-USER-ACCEPTANCE-2026-06-02`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530V-LIVE-2026-06-02`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530V-PCNT-SOURCE-BUILD-2026-06-02`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530U-LIVE-2026-06-02`, `SRC-ESP-IDF-PCNT`, `SRC-ESP-IDF-GPIO` | PF0530V keeps GPIO13/GPIO14/GPIO32 input-only and LCD GPIO21/GPIO22 display-only, changes the active source identity to `PF0530V`, declares `esp_driver_pcnt`, configures PCNT quadrature counting for GPIO13/GPIO14, emits menu steps from PCNT count deltas with four counts per step and a four-step poll cap, keeps switch polling/debounce with a 40 ms guard, and reports `cal=pcnt-v1`, `decoder=pcnt`, `irq=pcnt`, `poll_decoder=0`, `ENC_PCNT_READY`, and `ENC_PCNT_HB`. COM6 identity, 4 MB rollback backup, staged hashes, write-flash, separate verify-flash, reset/read-only readiness monitor, idle read-only monitor, transcript scan, and cleanup proof passed with zero crash/unsafe markers. After that gate, the user stated `ENCODER FUNCTIONALITY CONFIRMED AND APPROVED BY USER`, accepting PF0530V real LCD menu encoder functionality. | Post-confirmation transcript counts were not captured, so direction sign, counts-per-detent, quantified runaway tolerance, and short/long select count characterization remain open telemetry topics. | Use the flashed PF0530V image as the accepted functional LCD menu encoder image; run a separate read-only monitor only if transcript-count characterization is required. Do not reflash unless a new gate explicitly authorizes it. |
| Four-relay LCD visual-art menu | PF0530W-host-validation-accepted-art-visual-pending | firmware-visible visual-art source/build continuation, COM6 write/verify/readiness proof, ART-carousel source/build proof, and host-only validation packet | `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530W-HOST-ONLY-VALIDATION-2026-06-05`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530W-ART-CAROUSEL-2026-06-02`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530W-LIVE-2026-06-02`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530W-VISUAL-ART-2026-06-02`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530V-USER-ACCEPTANCE-2026-06-02`, `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530V-LIVE-2026-06-02`, `SRC-HITACHI-HD44780U-DDRAM-CGRAM-2026-05-31`, `SRC-NXP-HD44780-4X20-DDRAM-2026-05-31` | PF0530W preserves the PF0530V PCNT encoder path and closed safety surfaces, adds a generated `ART` page and seventh `art_panel` glyph bank, and Task 0148 source/build records a five-panel ART carousel. COM6 identity, 4 MB rollback backup, staged hashes, write-flash, separate verify-flash, read-only PF0530W readiness monitor, transcript scan, cleanup proof, and zero crash/unsafe markers passed for the written image. Task 0181 accepts host-only simulator/catalog/record validation: the ART page cycles all five panels, wraps, preserves `art_panel` focus/glyph bank, and exposes `bbs_lcd_pixel_preview.v1` metadata. | Physical ART page visual acceptance, current COM6/device state, and live ART-carousel telemetry remain unverified. | Use Task 0181 for host-only visual validation and PF0530W as the written/verify-flashed visual-art lineage. Next useful action is physical encoder navigation to the ART page with operator visual acceptance, optionally accompanied by read-only ART-page monitor evidence. Do not reflash or send serial commands unless a new Tier 3 gate explicitly authorizes it. |
| Remote LCD XBee solar client hardware-device stream | hardware-intake-audited-identity-open | parent coordination, private docs-only submodules, current submodule status, and Task 0183 parent intake audit | `SRC-LOCAL-REMOTE-LCD-XBEE-SOLAR-CLIENT-SCAFFOLD-2026-05-26`, `SRC-LOCAL-REMOTE-LCD-XBEE-SOLAR-CLIENT-PRIVATE-SUBMODULES-2026-05-26`, `SRC-LOCAL-REMOTE-LCD-XBEE-SOLAR-CLIENT-SEPARATE-HARDWARE-STREAM-2026-05-26`, `SRC-LOCAL-REMOTE-LCD-XBEE-SOLAR-CLIENT-HARDWARE-INTAKE-AUDIT-2026-06-05` | Seven private `rlxsc-*` hardware submodules exist and the lane is recorded as separate from ESP-NOW BBS, Win31/DOS-C, Gate F runtime, Gate G export, Gate H proof, mesh, BLE, network, relay, TFT, MicroSD, load, and mains work. Task 0183 audits the parent intake/status boundary, records fresh submodule status context, and keeps all candidate IC/source rows as reference-only. | Exact ESP32 board, LCD/backpack, encoder, cell, BMS/protection board, charger/power path, panel, XBee carrier, antenna, fuse/protection, enclosure, power budget, pin map, framework ADR, and read-only bench proof remain open. No wiring, charging, battery/solar connection, XBee serial/RF/writes, firmware, framework, live measurement, or bench action is accepted. | Start source-backed identity intake inside the private submodules, prioritizing cell, BMS/protection, charger/power path, panel, fuse/protection, enclosure, and current-limit evidence before board/interface or XBee bench action. |
