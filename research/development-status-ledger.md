...
| XBee lane | study-cli-added-blocked-live | source-backed study plus offline tooling | `SRC-DIGI-XBP9B-DPUT-001`, `SRC-DIGI-XBEE-PRO-900HP`, `SRC-DIGI-XBEE-900HP-USER-GUIDE`, `SRC-WAVESHARE-XBEE-USB-ADAPTER`, `SRC-LOCAL-XBEE-READONLY-PROBE-2026-05-18`, `SRC-LOCAL-XBEE-RADIO-STUDY-2026-05-29` | Candidate XBee and adapter sources exist; the new study CLI can inventory host/tool state without opening serial ports, compare readback/target JSON offline, and emit blocked write plans. | Adapter serial path, UART voltage, DIN/DOUT routing, settings backup, legal/antenna constraints, XCTU install proof, and read-only Tier A/Tier B evidence. | Use offline study tooling first; run read-only XBee bench proof only after current adapter identity is verified and a live gate is opened. |
