...
| Four-relay LCD visual-art PF0530W live proof | `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530W-VISUAL-ART-2026-06-02` and `SRC-LOCAL-FOUR-RELAY-KY040-BBS-LCD-MENU-PF0530W-LIVE-2026-06-02` record the firmware-visible LCD visual-art source/build and COM6 write/verify/readiness gate. PF0530W preserves the PF0530V PCNT encoder path and closed safety surfaces, adds an `ART` page, seventh `art_panel` glyph bank, and fixed 4x20 custom-character tile map, then passes redacted COM6 identity, rollback backup with private hashes retained only in ignored local evidence, write-flash, separate verify-flash, read-only PF0530W readiness monitor, transcript scan, cleanup proof, and zero crash/unsafe markers. Remaining gap: physical ART page visual acceptance and ART render telemetry are not captured because the read-only monitor stayed on HOME with no physical encoder input. |
