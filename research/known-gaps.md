...
- Custom wireless protocol Gate B and Gate C now have simulator-only proof for
  ...
  `COM9`/`COM6`/`COM7` peer remap matched accepted peer MACs. Firmware ABI,
  proposed `ADR-0005` analytics export policy, and a fresh live packet captured
  with `bridge-transcript.jsonl` still need owner review.
